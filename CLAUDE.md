...
Le socle TypeScript existe (T0a) : `apps/api`, `apps/worker`, `apps/backoffice`, `packages/contract`, `packages/db`, `packages/jobs`, sans table ni authentification. Le squelette iOS existe (T0b) : `ios/`, projet XcodeGen, écran d'état de `/health`. La démo est déployée (T1) : Supabase Cloud, Railway (`apps/*/Dockerfile`, réglages des services dans le README § 7, migrations en pré-déploiement), Vercel ; détail dans `README.md` § 7. T2 pose l'authentification (JWT ES256 vérifiés par JWKS, ADR-018), le module `identity` (table `profiles`) et l'onboarding iOS en wireframe. T3 pose le pipeline média (ADR-008) : module `media` (table `media`, upload présigné, ports `UnitOfWork` et `JobQueue`), jobs `process-image` et `purge-orphan-media` dans le worker (sharp), photo de profil. T4 pose la politique de visibilité (`apps/api/src/shared/domain/visibility.ts`, ADR-006) et le port transverse `RelationshipReader` (tables `follows` et `blocks`), `GET /v1/users/{username}`, la feature iOS `Profile`, un onglet Recherche provisoire (username exact, remplacé en T5) et un seed minimal (`SEED_VIEWER_USERNAME`). T5 pose le module `social` (follow / unfollow, listes d'abonnés paginées par curseur, `GET /v1/search/users` avec `pg_trgm`), le `SocialService` iOS (`Core/Social`), le bouton Suivre optimiste et l'onglet Recherche. T6a pose le module `posts` (`POST /v1/posts`, `GET /v1/posts/{id}`, `GET /v1/users/{id}/posts` par pages de 12, tables `posts` et `post_media`), le rate limit par route (`@fastify/rate-limit`), la file de publication persistée iOS (`Core/Posts/PublishQueue`, bandeau « Publication en cours »), le recadrage comme Instagram (`Core/Media/ImageCropper`), la feature `Create`, la grille du profil et la feature `Post` (détail). T6b pose le carrousel (1 à 10 photos au ratio de la première, `PublishQueue` multi-médias qui ne renvoie que les photos non envoyées), `DELETE /v1/posts/{id}` (suppression logique par l'auteur, médias détachés puis purgés, `post_count` décrémenté, `204` ou `404 post_not_found`) et le menu « … » du détail. Mettre à jour ce fichier au fur et à mesure du scaffolding.
