...
| P | Écrans |
|---|---|
| P0 | Connexion / inscription, onboarding, accueil (feed), détail d'un post, commentaires, création de post (sélection, recadrage simple, légende), profil (le mien, un autre), modification du profil, abonnés / abonnements, recherche, paramètres (compte privé, bloqués, suppression du compte), signalement |
...
- `.xcconfig` par environnement (`Local.xcconfig`, non versionné, avec un modèle versionné) : URL de l'API, URL Supabase, clé publique Supabase.
- `Info.plist` : `NSAllowsLocalNetworking`, `NSLocalNetworkUsageDescription`, `NSCameraUsageDescription`, `NSPhotoLibraryUsageDescription`, `NSMicrophoneUsageDescription` (reels).
